-- Stage 13.12.5 follow-up: can_delete / delete only for open entities.
...
begin
  ...
  if v_kind in ('topic', 'topic_selection') then
    ...
    if v_selection.status is distinct from 'open' then
      return false;
    end if;
  ...
  if v_kind in ('collection', 'group_collection') then
    ...
    if v_collection.status is distinct from 'open' then
      return false;
    end if;
