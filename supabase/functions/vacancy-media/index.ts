// Stage 17 P1 local vacancy-media Edge (deploy owner-gated).
...
Deno.serve(async (req) => {
  ...
  try {
      ...
      for (const row of rows) {
        ...
        try {
          ...
          const message = cleanupError instanceof Error
            ...
            : "cleanup_failed";
          await service.rpc("fail_vacancy_media_cleanup", {
            p_queue_id: queueId,
    ...
    if (action === "createUpload") {
      const vacancyId = asUuid(body.vacancyId);
      const contentType = asTrimmedString(body.contentType).toLowerCase();
      if (!vacancyId) throw new InputError("vacancyId required");
