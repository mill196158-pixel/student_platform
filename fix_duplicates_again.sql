-- ========================================
-- ИСПРАВЛЕНИЕ ДУБЛИРУЮЩИХСЯ ФУНКЦИЙ (ПОВТОРНО)
-- ========================================

-- 1. Показываем ВСЕ существующие версии функций
SELECT 
...
-- 2. Удаляем ВСЕ существующие версии функций
DROP FUNCTION IF EXISTS public.send_chat_message(uuid, text, text, uuid, text, uuid, uuid);
...
-- 3. Создаем ЕДИНСТВЕННУЮ версию send_chat_message (с поддержкой file_id)
...
BEGIN
  ...
  -- Если передан file_id, обновляем chat_files
...
-- 4. Создаем ЕДИНСТВЕННУЮ версию send_chat_message_for_login
...
GRANT EXECUTE ON FUNCTION public.send_chat_message(uuid, text, text, uuid, text, uuid, uuid) TO authenticated;
...
-- 7. Показываем финальные сигнатуры
SELECT 
    p.proname as function_name,
    pg_get_function_arguments(p.oid) as arguments,
    pg_get_function_result(p.oid) as return_type
