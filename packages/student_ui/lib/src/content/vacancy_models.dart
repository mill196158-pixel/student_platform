...
({String body, String requirements}) splitVacancyDescription(
    String description) {
  ...
  return (
    ...
    requirements:
        description.substring(idx + vacancyRequirementsMarker.length).trim(),
  );
}
...
class VacancyCardPayload {
  ...
  static VacancyCardPayload? tryParse(Map<String, dynamic>? json) {
    ...
    try {
      ...
      final descriptionFull = split.body.isNotEmpty
          ? split.body
          : (rawDescription.trim().isEmpty ? null : rawDescription.trim());

      final requirements =
          split.requirements.isNotEmpty ? split.requirements : null;
