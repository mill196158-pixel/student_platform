-- ========================================
-- ВОССТАНОВЛЕНИЕ ФУНКЦИЙ ОТПРАВКИ СООБЩЕНИЙ
-- ========================================

-- 1. Удаляем все существующие версии функций
...
DROP FUNCTION IF EXISTS public.send_chat_message_for_login(uuid, text, text, uuid, text, uuid);
DROP FUNCTION IF EXISTS public.send_chat_message_for_login(uuid, text, text, uuid, text);
DROP FUNCTION IF EXISTS public.send_chat_message_for_login(uuid, text, text, uuid);
...
-- 2. Создаем правильную версию send_chat_message
...
BEGIN
  ...
  -- Вставляем сообщение и получаем его ID
...
-- 3. Создаем правильную версию send_chat_message_for_login
...
GRANT EXECUTE ON FUNCTION public.send_chat_message_for_login(uuid, text, text, uuid, text, uuid) TO authenticated;
