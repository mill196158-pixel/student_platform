...
/// Test double that implements both local preview storage and remote gateway.
class _FailingRemoteImageStore
    implements AdminImageStore, AdminRemoteImageGateway {
  _FailingRemoteImageStore({this.failPaths = const {}});

  final Set<String> failPaths;
  final List<String> strictDeleteCalls = [];

  @override
  Uint8List? getBytes(String imageId) => null;

  @override
  String? mimeTypeOf(String imageId) => null;
  ...
  @override
  Uint8List? peekBytes(String? imagePath, {required String version}) => null;
  ...
  Future<void> deleteRemoteStrict(String imagePath) async {
    strictDeleteCalls.add(imagePath);
    if (failPaths.contains(imagePath)) {
      throw StateError('storage delete failed: $imagePath');
    }
  }
...
void main() {
  ...
  group('NewsEditorScreen archive UI', () {
    ...
    testWidgets(
      'storage delete failure queues cleanup and keeps post deleted',
      (tester) async {
        tester.view.physicalSize = const Size(1440, 1000);
        tester.view.devicePixelRatio = 1;
        addTearDown(tester.view.resetPhysicalSize);
        ...
        const orphanPath = 'news/orphan-fail.jpg';
        const keptPath = 'news/kept-ok.jpg';
        final repo = LocalNewsRepository(
          seed: [
            _item(
              id: 'arch-fail',
        ...
        final media = _FailingRemoteImageStore(failPaths: {orphanPath});

        await tester.pumpWidget(
          MaterialApp(
            home: Scaffold(
              body: NewsEditorScreen(repository: repo, imageStore: media),
            ),
          ),
        );
        await tester.pumpAndSettle();
        ...
        await tester.tap(find.byTooltip('Архив'));
        await tester.pumpAndSettle();
        await tester.tap(find.text('Архив с orphan').first);
        await tester.pumpAndSettle();

        await tester.tap(find.byKey(const ValueKey('delete-archived-forever')));
        await tester.pumpAndSettle();
        await tester.tap(find.text('Удалить навсегда'));
        await tester.pumpAndSettle();

        expect(
          find.text('Новость удалена, очистка файла будет повторена'),
          findsOneWidget,
        );
        expect(media.strictDeleteCalls, [orphanPath]);
        expect(repo.pendingMediaCleanup, hasLength(1));
        expect(repo.pendingMediaCleanup.single['object_path'], orphanPath);
        expect(
          repo.pendingMediaCleanup.single['source_news_post_id'],
          'arch-fail',
        );
        expect(
          repo.pendingMediaCleanup.single['source_title'],
          'Архив с orphan',
        );
        expect(await repo.listNews(), hasLength(1));
        expect((await repo.listNews()).single.id, 'arch-ok');
        expect(
          repo.pendingMediaCleanup.any((e) => e['object_path'] == keptPath),
          isFalse,
        );
      },
    );
