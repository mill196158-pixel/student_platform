...
void main() {
  ...
  testWidgets('StudentNewsStorySheet shows server image bytes for overlay', (
    tester,
  ) async {
    final serverPng = Uint8List.fromList([
      0x89,
      0x50,
      0x4E,
      0x47,
          ...
          body: StudentNewsStorySheet(
            news: [
              StudentHomeNews(
                ...
                title: 'Server photo',
                subtitle: 'sub',
                body: 'Full body from published news',
                icon: Icons.folder_copy_outlined,
                gradientColors: const [Color(0xFF7367F0), Color(0xFFB784F7)],
                variant: StudentHomeNewsVariant.imageOverlay,
                imageBytes: serverPng,
                publishedAt: DateTime(2026, 7, 22),
              ),
            ],
          ),
    ...
    await tester.pumpAndSettle();
    expect(find.byType(Image), findsWidgets);
    expect(find.text('Server photo'), findsOneWidget);
    expect(find.byIcon(Icons.folder_copy_outlined), findsNothing);
  });
}
