-- Удаляем старую функцию и создаем новую get_chat_messages_for_team
-- ========================================

-- 1. Удаляем старую функцию
DROP FUNCTION IF EXISTS get_chat_messages_for_team(uuid);

-- 2. Создаем новую функцию с file_id
CREATE OR REPLACE FUNCTION get_chat_messages_for_team(p_team_id uuid)
RETURNS TABLE(
    id uuid, 
    chat_id uuid, 
    author_id uuid, 
